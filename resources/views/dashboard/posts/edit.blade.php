...
@section('content')
    <div class="text-center p-12 gap-12">
        ...
        <form action="{{ route('posts.update' , $post) }}" method="post" name="form" enctype="multipart/form-data">
            @method('PATCH')
            @csrf
                ...
                <div>
                    ...
                    <select id="countries"
                        ...
                        @foreach($statuses as $status)
                            ...
                            <option
                                {{ $status == $post->status->value ? 'selected' :'' }} value="{{ $status }}">{{ $status }}</option>
                        @endforeach
                    </select>
                    @error('status')
                    <div class="alert alert-danger text-red-600">{{ $message }}</div>
                ...
                <div>
                    ...
                    <select id="countries"
                        ...
                        @forelse($categories as $category)
                            ...
                            <option
                                {{ !is_null($post->categories->find($category)) ? 'selected' : '' }}
                    ...
                    <div>
                        <label class="block text-left" style="max-width: 300px">
                            ...
                            <select name="tags[]" class="form-multiselect block w-full mt-1" multiple>
                                @foreach($tags as $tag)
                                    <option
                                        {{ !is_null($post->tags->find($tag)) ? 'selected' : '' }}
